...
// PATCH /api/department/[id]/status — toggle is_active between 1 and 0
export async function PATCH(
  ...
  try {
    ...
    return NextResponse.json(
      { message: `Department berhasil di${newStatus === 1 ? "aktifkan" : "nonaktifkan"}`, department: updated },
      { status: 200 }
    );
  } catch (error) {
    console.error("PATCH /api/department/[id]/status error:", error);
